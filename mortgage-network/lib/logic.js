...
/**
  * A transaction processor function description
  * @param {com.devb.mortgage.GenerateULDD} tx parameter-name 
  * @transaction
  */
 function generateUldd(tx) {
    var assetRegistry;  
    var id = tx.fnmOverlay.loanId;
    return getAssetRegistry('com.devb.mortgage.ULDD30')
        .then(function(ar) {
            assetRegistry = ar;
            return assetRegistry.get(id);
        })
        .then(function(asset) {
            return assetRegistry.update(tx);
        });
  }  